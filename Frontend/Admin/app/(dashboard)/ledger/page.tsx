...
export default function LedgerPage() {
  ...
  const loadLedger = useCallback(async () => {
    ...
    setError(null);
    try {
      const data = await fetchAllLedgerRows();
      setRows(data);
    } catch (err) {
      const msg =
  ...
  const filtered = useMemo(
    ...
    [rows, search, walletFilter, sourceFilter, entryFilter, dateFrom, dateTo],
  );

  const totals = useMemo(() => computeLedgerTotals(filtered), [filtered]);
  ...
  const exportCsv = () => {
    ...
    a.href = url;
    a.download = `ledger-${todayYmd()}.csv`;
    ...
    <div className="space-y-6 pb-8">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-[var(--text-muted)]">
              Double-entry log of credits &amp; debits across DIRECT and TEAM wallets.
            </p>
          </div>
        </div>
        <Button
      ...
      {error && (
        <div className="rounded-xl border border-rose-200 bg-rose-50 px-4 py-3 flex items-start gap-2">
          ...
          <div>
            <p className="text-sm font-semibold text-rose-800">Could not load ledger</p>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="rounded-2xl border border-emerald-200/70 bg-gradient-to-br from-emerald-50/70 to-white p-5 shadow-[var(--shadow-sm)]">
          ...
          <p className="mt-3 text-2xl font-bold text-[var(--text-primary)] tabular-nums">
            {loading ? "…" : formatINR(totals.credit)}
          </p>
        </div>
        <div className="rounded-2xl border border-rose-200/70 bg-gradient-to-br from-rose-50/70 to-white p-5 shadow-[var(--shadow-sm)]">
          ...
          <p className="mt-3 text-2xl font-bold text-[var(--text-primary)] tabular-nums">
            {loading ? "…" : formatINR(totals.debit)}
          </p>
        </div>
      ...
      <Card padding="md">
        <div className="grid grid-cols-1 md:grid-cols-6 gap-3">
          <div className="md:col-span-2">
            <Input
              placeholder="Search user, SPF code, ref, description…"
          ...
          <div className="md:col-span-4 flex flex-wrap items-end gap-2 justify-between">
            <div className="flex flex-wrap items-center gap-1.5">
              <span className="inline-flex items-center gap-1 text-[11px] font-semibold text-[var(--text-muted)] px-2">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left">
                ...
                <tbody className="divide-y divide-[var(--border-subtle)]">
                  {pageData.map((e) => (
                    <tr key={`${e.wallet_type}-${e.id}`} className="hover:bg-slate-50/80 transition-colors">
                      <td className="px-5 py-4 whitespace-nowrap">
                        <p className="text-sm text-[var(--text-primary)]">
                          {formatDate(e.created_at, false)}
                        </p>
